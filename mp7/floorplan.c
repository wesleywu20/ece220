...
int is_leaf_node(node_t* ptr) {
  ...
  /* if current node has no children, return 1; else, return 0 */
...
int is_internal_node(node_t* ptr) {
  ...
  /* if current node has no children, return 0; else, return 1 */
  if (ptr->left == NULL && ptr->right == NULL) {
    return 0;
  }
  return 1;
...
int is_in_subtree(node_t* a, node_t* b) {
  if (a == b) { // if a and b point to the same node -> they are the same subtree -> return 1
    return 1;
  }
  if (a->left == NULL || a->right == NULL) { // if the current node has no children, it is a leaf node -> the depth of the tree has been searched -> return 0
    return 0;
  }
  if (a->left == b || a->right == b) { // if either of the current node's children equal the node being searched for, return 1
    return 1;
  }
  int left = is_in_subtree(a->left, b); // try to find b in the left subtree of the current node
  int right = is_in_subtree(a->right, b); // try to find b in the right subtree of the current node
  return (left || right); // return whether b has been found in either subtree
}
...
void rotate(node_t* ptr) {
  int temp = ptr->module->w; // store the module's width into temp
  ptr->module->w = ptr->module->h; // assign the module's height to be its width
  ptr->module->h = temp; // reassign the temp (holding the module's width) to the module's height
}
...
void recut(node_t* ptr) {
  ...
  if (ptr->cutline == V){ // if the node's cutline is vertical, change it to horizontal
    ptr->cutline = H;
    return;
  }
  ptr->cutline = V; // otherwise, it is horizontal -> change it to vertical
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  module_t* temp = a->module; // store a's module into temp
  a->module = b->module; // assign b's module to be a's module
  b->module = temp; // reassign the module pointed to by temp (a's module) to be b's module

}

/*
  This method swaps the parents of nodes a and node b in the swap_topology method,
  AFTER it is asserted that neither of node a or node b have NULL-defined parents
  (i.e. it is safe to assume parents exist)
*/
void swap_parents(node_t* a, node_t* b) {
    node_t* temp = a->parent; // store a's parent into temp
    a->parent = b->parent; // assign b's parent to be a's parent
    b->parent = temp; // reassign temp (holding a's parent) to be b's parent
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  /* case 1: if node a is a left child and the node b is a right child */
  if (a->parent->left == a && b->parent->right == b) {
    // first swap parents
    swap_parents(a, b);
    // update node links
    b->parent->left = b;
  ...
  /* case 2: if node b is a left child and the node a is a right child */
  else if (b->parent->left == b && a->parent->right == a) {
    // first swap parents
    swap_parents(a, b);
    // update node links
    a->parent->left = a;
  ...
  /* case 3: if both nodes are left childs */
  else if (a->parent->left == a && b->parent->left == b) {
    // first swap parents
    swap_parents(a, b);
    // update node links
    a->parent->left = a;
  ...
  /* case 4: if both nodes are right childs */
  else {
    // first swap parents
    swap_parents(a, b);
    // update node links
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  if(ptr == NULL) return;
  postfix_traversal(ptr->left, nth, expression); // recurse to the left subtree
  postfix_traversal(ptr->right, nth, expression); // recurse to the right subtree
  int idx = *nth; // dereference the index of the current expression unit and store it into the the idx variable
  // process the data part of the root element 
  if (ptr->module != NULL) { // if the node has a module, assign the module to the current expression unit
    expression[idx].module = ptr->module; 
    expression[idx].cutline = UNDEFINED_CUTLINE; // since this node is a module, it won't have a cutline -> define the current expression unit's cutline as "UNDEFINED_CUTLINE"
    *nth = *nth + 1; // update the index pointer
  }
  else { // otherwise, it has a cutline, so assign that cutline to the current expression unit
    expression[idx].cutline = ptr->cutline;
    expression[idx].module = NULL; // since this node is a cutline, it won't have a module -> define the current expression unit's module as NULL
    *nth = *nth + 1; // update the index pointer
  }
}
...
{
  int resource = 0; // initialize a variable to hold the value of the resource of the module (if it exists)
  if (ptr == NULL) { // if the module of the current node doesn't exist, return 0
    return 0;
  }
  if (ptr->left == NULL || ptr->right == NULL) { // if it is a leaf node, assign resource the value of this node's resource value
    resource = ptr->module->resource; 
  }
  return resource + get_total_resource(ptr->left) + get_total_resource(ptr->right); // return this modules resource value and add the left subtree's and right subtree's resource values to the tree
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // base case: n at end (num_modules - 1)
  if (n == num_modules - 1) {
    // make left child node w/ final module
    node_t *new_node = (node_t *)malloc(sizeof(node_t));
    new_node->cutline = UNDEFINED_CUTLINE;
    new_node->module = modules + n;
    new_node->left = NULL;
    new_node->right = NULL;
    // link it to par
    new_node->parent = par;
  ...
  // step 1: make vertical cut node
  node_t *vtcl_cut = (node_t *)malloc(sizeof(node_t));
  vtcl_cut->module = NULL;
  vtcl_cut->cutline = V;

  // step 2: make right child (with module n)
  node_t *right_child = (node_t *)malloc(sizeof(node_t));
  right_child->cutline = UNDEFINED_CUTLINE;
  right_child->parent = vtcl_cut;
  right_child->left = NULL;
  right_child->right = NULL;
  right_child->module = modules + n;
  vtcl_cut->right = right_child;

  //step 3: recurse left with n + 1, return the vertical cut node
  vtcl_cut->left = init_slicing_tree(vtcl_cut, n + 1);
  return vtcl_cut;
}
